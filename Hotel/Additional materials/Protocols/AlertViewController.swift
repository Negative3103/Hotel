...
extension AlertViewController where Self: UIViewController {
    func addErrorAlertView(error: (APIError, String?), completion: (() -> ())? = nil) {
        switch error.0 {
        case .requestFailed:
            showAlert(title: "Нет подключения к интернету", message: "Проверьте подключение к интернету и повторите снова", buttonAction: nil)
        case .notAuthorized:
            showAlert(title: "Нет авторизации", message: "Для доступа к данному контенту необходимо авторизоваться", buttonAction: nil)

        case .fromMessage:
            guard let message = error.1 else { return }
            showAlert(title: message, message: "", buttonAction: nil)
        default:
            showAlert(title: "Ошибка", message: "Ошибка сервера", buttonAction: nil)
        }
    }
